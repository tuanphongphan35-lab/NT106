...
namespace Login
{
    public partial class ChatForm : Form
    {
        ...
        private void HandleCallResponse(string[] parts)
        {
            ...
            this.Invoke((MethodInvoker)delegate
            {
                if (decision == "REJECT") MessageBox.Show($"{responder} đang bận hoặc đã từ chối cuộc gọi.");
                // else if (decision == "ACCEPT") MessageBox.Show($"{responder} đã bắt máy!"); 
            });
        }
        ...
        private void XuLySauKhiLoadDanhSach()
        {
            ...
            {
                // TRƯỜNG HỢP KHÔNG CÓ BẠN BÈ (Newbie):
                HienThiManHinhChaoMung();
            }
        }

        // --- HÀM 2: HIỂN THỊ MÀN HÌNH CHỜ KHI KHÔNG CÓ BẠN ---
        private void HienThiManHinhChaoMung()
        {
            // 1. Reset tên phòng
            lblTenPhong.Texts = "Chào mừng bạn mới!";
            _nguoiDangChat = ""; // Không chat với ai cả

            // 2. Xóa khung chat cũ
            txtChatBox.Controls.Clear();

            // 3. Tạo một thông báo hướng dẫn to đẹp giữa màn hình
            Label lblHuongDan = new Label();
            lblHuongDan.Text = "Chào mừng đến với ElicChat!\n\nBạn chưa có bạn bè nào.\nHãy nhấn nút 'Kính lúp' hoặc 'Danh bạ'\nđể tìm và kết bạn ngay nhé!";
            lblHuongDan.Font = new Font("Segoe UI", 14, FontStyle.Bold);
            lblHuongDan.ForeColor = Color.Gray;
            lblHuongDan.AutoSize = false;
            lblHuongDan.TextAlign = ContentAlignment.MiddleCenter;
            lblHuongDan.Dock = DockStyle.Fill; // Lấp đầy khung chat

            txtChatBox.Controls.Add(lblHuongDan);

            // 4. (Tùy chọn) Khóa ô nhập tin nhắn lại cho đến khi họ chọn bạn
            txtInput.Enabled = false;
            // btnSend.Enabled = false; 
        }

        private void lblTenPhong__TextChanged_1(object sender, EventArgs e)
        {
